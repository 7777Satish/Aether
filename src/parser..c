...
TokenType getTokenType(char* word){
    if(isType(word)) return TOKEN_TYPE;
    else if(isKeyword(word)) return TOKEN_KEYWORD;
    else if(isIdentifier(word)) return TOKEN_IDENTIFIER;
    else if(isConstant(word)) return TOKEN_KEYWORD;
    else if(isOperator(word)) return TOKEN_OPERATOR;
    else if(isPreprocessor_directive(word)) return TOKEN_PREPROCESSOR_DERIVATIVE;
    else if(isPunctuation(word)) return TOKEN_PUNCTUATION;
    else if(isNumber(word)) return TOKEN_NUMBER;
...
Token* createToken(char* word, int custom, SDL_Color color){
    ...
    if(custom){
        ...
        if(type==TOKEN_TYPE) clr = (SDL_Color){198, 120, 221, 255};
        else if(type==TOKEN_KEYWORD) clr = (SDL_Color){198, 120, 221, 255};
        else if(type==TOKEN_IDENTIFIER) clr = (SDL_Color){220, 220, 170, 255};
        else if(type==TOKEN_OPERATOR) clr = (SDL_Color){212, 212, 212, 255};
        else if(type==TOKEN_PREPROCESSOR_DERIVATIVE) clr = (SDL_Color){155, 155, 255, 255};
        else if(type==TOKEN_PUNCTUATION) clr = (SDL_Color){128, 200, 255, 255};
        else if(type==TOKEN_NUMBER) clr = (SDL_Color){181, 206, 168, 255};
        else clr = (SDL_Color){255, 255, 255, 255};
    }
...
FileLine* parseText(char* content){
    ...
    while (c != '\0')
    {
        ...
        if(!isInString && !isInComment && strlen(currentWord)!=0 && ((c==' ') || isPunctuation(tempC) || isOperator(tempC))){
            ...
            Token *t2 = NULL;
            if(c==' ') t2 = createToken(tempC, 1, white);
            else t2 = createToken(tempC, 0, white);
            
            t2->prev = lastToken;
            t2->next = NULL;

            if(lastToken) lastToken->next = t2;
            lastToken = t2;
